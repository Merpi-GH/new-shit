...
class WorkspaceBloc extends Bloc<WorkspaceEvent, WorkspaceState> {
  ...
  WorkspaceBloc(this.marketRepo) : super(WorkspaceState.initial()) {
    ...
    on<PublishMarket>(_publishMarket);
    on<UnPublishMarket>(_unPublishMarket);
    on<DeactivateMarket>(_deactivateMarket);
  }
  ...
  _getStores(LoadStores event, Emitter<WorkspaceState> emit) async {
    ...
    try {
      ...
      if (res is Success) {
        ...
        final storesList =
            initList.map((e) => MarketModel.fromJson(Map<String, dynamic>.from(e))).toList();
  ...
  _publishMarket(PublishMarket event, Emitter<WorkspaceState> emit) async {
    emit(state.copyWith(status: CWSStatus.loading));
    try {
      final res = await marketRepo.queueMarket(event.marketId);
      if (res is Success) {
        // بعد از انتشار، لیست را دوباره بارگذاری می‌کنیم
        final listRes = await marketRepo.getMarketList();
        if (listRes is Success) {
          final initList = listRes.response as List;
          final storesList = initList
              .map((e) => MarketModel.fromJson(Map<String, dynamic>.from(e)))
              .toList();
          emit(state.copyWith(
              status: CWSStatus.success, storesList: storesList));
        } else {
          emit(state.copyWith(status: CWSStatus.success));
        }
      } else {
  ...
  _unPublishMarket(UnPublishMarket event, Emitter<WorkspaceState> emit) async {
    emit(state.copyWith(status: CWSStatus.loading));
    try {
      // عدم انتشار: از همان endpoint inactive استفاده می‌شود یا queue با وضعیت not_published
      final res = await marketRepo.inactiveMarket(event.marketId);
      if (res is Success) {
        final listRes = await marketRepo.getMarketList();
        if (listRes is Success) {
          final initList = listRes.response as List;
  ...
  _deactivateMarket(DeactivateMarket event, Emitter<WorkspaceState> emit) async {
    emit(state.copyWith(status: CWSStatus.loading));
    try {
      final res = await marketRepo.inactiveMarket(event.marketId);
      if (res is Success) {
